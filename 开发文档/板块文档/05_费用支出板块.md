...
## 模块概述
费用支出板块专注于分析各维度的费用支出情况，包含两个核心图表：
1. **费用率分析图表**：通过双Y轴复合图表展示费用金额与费用率的综合分析
2. **费用结余分析图表**（2025-12-22新增）：展示费用结余金额和费用率超支情况，直观反映与预算目标的偏差

支持三级机构/客户类别/业务类型三个维度的切换分析，帮助快速识别费用管理重点区域和费用控制效果。

## 核心功能

### 1. 费用率分析图表（2025-12-21优化）
- **图表类型**: 双Y轴复合图表
...
### 2. 费用结余分析图表（2025-12-22新增）
- **图表类型**: 双Y轴复合图表
  - 左Y轴：费用结余额柱状图（正值红色，负值绿色）
  - 右Y轴：费用率超支折线图（深蓝色`#0070c0`）
- **核心指标**:
  - 费用结余额（万元）= 签单保费 × (实际费用率 - 费用率阈值)
    - 负值：费用节约（绿色柱）
    - 正值：费用超支（红色柱）
  - 费用率超支（百分点）= 实际费用率 - 费用率阈值
    ...
    - 正值：超出阈值
- **预警线系统**:
  - 0轴预算基准线（黑色实线）：费用结余的分界线
  - 0%费用率阈值线（黄色虚线）：14%阈值标准线
- **配色方案**:
  - 费用结余额柱：红色`#c00000`（超支）/ 绿色`#00b050`（节约）
  - 费用率超支线：深蓝色`#0070c0`
  - 预算基准线：黑色`#333333`
  - 阈值线：黄色`#ffc000`
- **业务价值**:
...
5. **费用结余计算**（2025-12-22新增）:
   ...
   - 计算费用率超支 = 实际费用率 - 预算阈值
   - 计算费用结余额 = 签单保费 × (实际费用率/100 - 预算阈值/100)
6. **排序**: 按费用金额降序排列
7. **极值处理**: 对极端费用率值进行归一化
